...
This report explains the design and development of the Hospital Patient and Appointment Management System, built using C++ for the CST209 Object-Oriented Programming course. The system allows users to register patients, assign doctors, manage a medical service catalogue, schedule appointments, and generate billing records. To ensure smooth operation, a multi-file project structure was used, consisting of seven header files and seven source files. Core object-oriented principles such as abstract base classes, inheritance, polymorphism, and composition were applied throughout the development. Modern C++ features, including smart pointers for automatic memory management, custom exceptions for safe error handling, and the Standard Template Library (STL) for efficient data storage, were also implemented. This report will walk through the rationale behind specific STL container choices, how the inheritance and polymorphism architecture functions, and how static functions and advanced features improve the overall system.

---

## 1. Introduction

Managing a hospital involves tracking many moving parts—patients, doctors, medical services, and appointments all need to interact perfectly. The goal of this project was to use C++ object-oriented programming to model these real-world healthcare interactions while ensuring data safety and providing a smooth user experience.

The project is organized across seven different classes: `Person`, `Patient`, `Doctor`, `MedicalService`, `AppointmentRecord`, `HospitalSystem`, and `InputValidator`. Each class has its own separate header (`.h`) and implementation (`.cpp`) file to maintain a clean codebase. The application is executed from `main.cpp`, which provides a user-friendly 18-option interactive command-line menu. Dynamic selection menus, strict input validation, and comprehensive error handling were included so the program will not crash if incorrect input is provided.

The following sections address the three main documentation requirements from the assignment: the use of the STL, the rationale behind the inheritance and polymorphism design, and the use of static functions alongside other advanced components.

---

## 2. STL Explanation

The Standard Template Library (STL) was used heavily throughout this project. Instead of relying on a single container for everything, different STL containers were carefully chosen based on what made the most sense for the specific data being stored.

### 2.1 std::vector

The `std::vector` container was used in several different ways. In the `HospitalSystem` class, a vector of smart pointers (`std::vector<std::unique_ptr<Person>>`) stores all the registered Patients and Doctors together. A vector was chosen here because the system frequently needs to loop through all users (for example, when displaying the "View All Users" menu). Vectors store items next to each other in memory, making this kind of looping very fast and efficient. Additionally, vectors automatically resize themselves when users are added or removed, eliminating the need to manually manage arrays.

In the `Patient` class, a `std::vector<AppointmentRecord>` stores a patient's medical history. This is a "composition" relationship—the vector literally owns the appointment records. If a patient is deleted from the system, their appointment history is automatically deleted with them, which perfectly mimics how it should work in real life. 

The `Doctor` class also uses `std::vector<std::string>` to track qualifications and a list of currently assigned patients. When an appointment is finished, the C++ erase-remove idiom is used to quickly remove that patient's ID from the doctor's active list.

### 2.2 std::unordered_map

For the medical service catalogue, a `std::unordered_map<std::string, MedicalService>` was utilized. This was the ideal choice because every medical service has a unique code (like "XRAY01" or "CONS01"). An unordered map functions like a hash table, meaning the system can look up a service code almost instantly—in constant time. In a busy hospital where services are constantly looked up for scheduling and billing, this ensures the program never slows down, regardless of how many services are added to the catalogue. It also makes checking if a service already exists before adding a new one highly efficient.

### 2.3 std::unique_ptr and Smart Pointer Usage

Instead of using traditional raw pointers, `std::unique_ptr<Person>` was used for the main user list. This approach provides automatic memory management. When a user is removed from the vector, or when the program closes, the `unique_ptr` automatically cleans up the memory. This guarantees that the program will not suffer from memory leaks.

### 2.4 File I/O with STL Streams

To ensure data is not lost when the program closes, `std::ofstream` was used to save the data to a text file, and `std::ifstream` was used to load it back. The system saves everything using a pipe-delimited format (using `|` to separate fields) and organizes it into sections. When loading the file back, a custom lambda function using `std::istringstream` cleanly splits the text back into individual data points.
...
The inheritance structure was designed based on the fact that patients and doctors share many basic personal details (like names, IDs, and contact info), but they have completely different roles in the hospital. 

A `Person` class was created to serve as an Abstract Base Class. It holds all the common personal details to avoid code duplication. By adding a "pure virtual function" called `virtual void displayRole() const = 0;`, it was made impossible to instantiate a generic "Person." Any user in the system *must* be either a Patient or a Doctor.

The `Patient` class inherits from `Person` (an "IS-A" relationship). It takes all the basic details from `Person` and adds specific patient data like insurance info, blood type, and appointment history. The `Doctor` class also inherits from `Person`, adding doctor-specific details like specialization, clinic code, and the number of patients seen. Both classes use member initializer lists in their constructors to pass the basic details up to the `Person` class safely.

### 3.2 Runtime Polymorphism

Runtime polymorphism is the mechanism that allows the system to treat both Patients and Doctors uniformly as "Users" while still knowing exactly how they should individually behave.

Because everyone is stored in a single `std::vector<std::unique_ptr<Person>>`, the system can loop through the entire list when the "View All Users" option is selected. As it loops, it calls `p->displayRole()` on every person. Through C++ runtime polymorphism (dynamic dispatch), the program checks exactly what kind of person it is at runtime. If it is a Patient, it prints their medical history and billing info. If it is a Doctor, it prints their specialization and qualifications. 

A virtual destructor (`virtual ~Person()`) was also included in the base class. This is absolutely mandatory because it ensures that when a Person pointer is deleted, it correctly calls the Patient or Doctor destructor first, preventing resource leaks. Finally, the `dynamic_cast` operator was used whenever there was a specific need to treat a Person as a Patient (such as when scheduling an appointment) to maintain type safety and prevent crashes.

### 3.3 Compile-Time Polymorphism

Compile-time polymorphism was also implemented by "overloading" constructors. For example, the `MedicalService` class has four different constructors. Depending on how much information is provided when creating a service, the compiler knows exactly which constructor to execute. The `==` operator was also overloaded so two medical services can be easily compared by simply checking their service codes.
...
Static members were used to manage data that belongs to the "whole system" rather than just one specific object. 

For instance, the `Patient` class has a static integer called `patientCount`. It increments when a patient is created and decrements when they are removed, providing a live, global count of patients for the System Summary screen. A static `nextPatientNumber` combined with a static `generatePatientId()` function is used to automatically generate unique IDs like "PAT1001" and "PAT1002". By making this static, the system guarantees that no two patients will ever receive the same ID, even if they are registered simultaneously. The `Doctor` and `AppointmentRecord` classes utilize the same static pattern to generate their respective IDs.

### 4.2 Custom Exception Hierarchy

To prevent the program from crashing upon invalid user input, a custom, three-level exception hierarchy was built. A `HospitalException` class was created that inherits from the standard `std::exception`. Then, two more specific exceptions (`DuplicateIdException` and `NotFoundException`) were created to inherit from `HospitalException`. 

In `main.cpp`, the entire menu loop is wrapped in a `try-catch` block. If a lookup is performed for a patient that does not exist, the system throws a `NotFoundException`. The catch block intercepts it, prints a friendly error message, and safely returns the user to the main menu instead of abruptly terminating the application.

### 4.3 Custom Template Library (InputValidator)

To ensure input menus are entirely robust, a custom template library called `InputValidator.h` was written. It uses an advanced C++ feature called a generic function template (`template <typename T>`) to accept any type of input (like integers or decimals). It continuously prompts the user until valid input is provided. An explicit template specialization for `std::string` was also created so that it uses `std::getline` (which allows spaces in names) instead of standard `cin`.

### 4.4 Scoped Enumerations

Scoped enumerations (`enum class`) were used for attributes like `InsuranceTier` and `AppointmentType`. This provides strong type safety and prevents bugs where raw integers might be accidentally mixed up. Switch statements on these enums are used to automatically apply 30% emergency surcharges or insurance discounts during billing calculations.

### 4.5 Composition and Aggregation Relationships

The system carefully distinguishes between composition (strong ownership) and aggregation (weak reference) relationships. The `AppointmentRecord` class contains a `MedicalService` object by value (composition), meaning the service data is embedded directly within the appointment and destroyed with it. The `Patient` class contains a vector of `AppointmentRecord` objects (also composition). In contrast, the `AppointmentRecord` stores only a `doctorId` string (aggregation), creating a weak reference to the Doctor. When a Doctor is removed from the system, the `removeUser()` method in `HospitalSystem` iterates through all patients and calls `clearDoctorFromAppointments()` to null out the dangling doctor references, while preserving the appointment data itself. This cleanup logic explicitly demonstrates an understanding of the difference between composition and aggregation ownership semantics.

### 4.6 Dynamic Interactive Selection Menus

Going beyond a standard text-input system, the application implements dynamic, context-aware selection menus. When scheduling an appointment, the system presents numbered lists of available patients, services, and doctors rather than requiring the user to memorize and type raw IDs. When removing a medical service, the system uses the `checkServiceRemovable()` method to pre-scan all active appointments and displays [BLOCKED] or SAFE TO REMOVE directly on the menu. When removing a doctor with active appointments, the system presents the blocking appointments with patient contact information and offers an option to reassign all appointments to a different doctor before proceeding with deletion. These features elevate the application from a basic academic exercise to an industry-grade user experience.
